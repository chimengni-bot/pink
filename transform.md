...
```
animation-play-state: paused;只能单独写
...
### 一句话
根据"**子元素的状态**"来选择"**父元素**"，是 CSS 里第一个真正的"父选择器"。
...
### 例子
```css
/* 鼠标悬停在第 1 个 .item 上时，修改它父亲 ul 的网格列宽 */
.box ul:has(.item:nth-child(1):hover) {
  grid-template-columns: 2fr 1fr 1fr 1fr;
}
```

读法：选中 `.box` 下面的 `ul`，**前提是** 这个 `ul` 里有一个 `.item:nth-child(1)` 正被鼠标悬停。

### 为什么重要
- 以前的 CSS 只能从父选到子（`父 子`），**不能反过来**
- `:has()` 让我们能根据子元素状态反向修改父元素，再也不用写 JS
- 常见用法：鼠标悬停子项时让兄弟项变窄、表单里有错误时让外框变红、卡片里有图片时调整布局

### 兼容性
现代浏览器（Chrome 105+ / Safari 15.4+ / Firefox 121+）都支持，老 IE 没有。

# 109.动效案例4-漂亮的卡片动画效果
## 核心三步

1. **多个元素**，利用**定位**分别移动到目标位置
2. 元素开始时用 `rotateX` / `rotateY` 拉伸（或 `skew` 斜切）
3. 鼠标经过时**去掉倾斜效果**，用**平移**微调位置
...
```css
-webkit-box-reflect: 倒影方向 倒影距离 倒影图像;
```
### ① 倒影方向（必写）
...
### ③ 倒影图像（可选）
常用线性渐变 linear-gradient() 实现从清晰到透明的过渡
...
**效果**：鼠标指到哪个盒子，**其余的都变形缩小**，当前那个保持不变 → 突出聚焦感
...
| 写法 | 含义 |
|---|---|
| `li:not(:first-child)` | 除第一个 li 外的所有 li |
| `li:not(:last-child)` | 除最后一个 li 外的所有 li |
| `input:not([type="submit"])` | 除提交按钮外的所有输入框 |
| `a:not(.active)` | 没有 active 类的所有 a 标签 |
| `div:not(.box):not(.card)` | 既不是 box 也不是 card 的 div |

---

### 💡 小贴士

1. 括号里可以放**任何选择器**（类、ID、属性、伪类都行）
2. 可以**链式使用** `:not(A):not(B)` 排除多个
3. 比传统写法**少写一堆类名**，代码更优雅

#### 对比

```css
/* 笨办法：给除第一个外的每个 li 加 class */
.li-other { margin-top: 10px; }
